...
function StatCard({label,value,sub,accent}) {
  return (
    <div style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:8,padding:"20px 24px",display:"flex",flexDirection:"column",gap:4,borderTop:`3px solid ${accent||"#2563eb"}`}}>
      ...
      <div style={{fontSize:36,fontWeight:800,color:accent||"#0f172a",lineHeight:1,fontFamily:"'Bebas Neue','Arial Narrow',sans-serif",letterSpacing:"0.02em"}}>{value}</div>
...
function Modal({open,onClose,children,wide}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:12,padding:32,width:"100%",maxWidth:wide?720:520,maxHeight:"92vh",overflowY:"auto"}}>
        {children}
      </div>
    </div>
  );
...
function Inp({label,...props}) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      ...
      <input {...props} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:6,padding:"9px 12px",color:"#0f172a",fontSize:14,outline:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box",...(props.style||{})}}/>
    </div>
  );
}
function Sel({label,children,...props}) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {label&&<label style={{fontSize:11,color:"#64748b",letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'Courier New',monospace"}}>{label}</label>}
      <select {...props} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:6,padding:"9px 12px",color:"#0f172a",fontSize:14,outline:"none",fontFamily:"inherit",width:"100%",boxSizing:"border-box"}}>{children}</select>
    </div>
  );
}
function Spinner() {
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 0",gap:16}}>
      <div style={{width:32,height:32,border:"3px solid #1e3a5f",borderTopColor:"#2563eb",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
      <div style={{fontSize:13,color:"#64748b",fontFamily:"'Courier New',monospace",letterSpacing:"0.08em"}}>LOADING POSTS...</div>
...
function ErrorBanner({msg,onRetry}) {
  return (
    <div style={{background:"#fef2f2",border:"1px solid #7f1d1d",borderRadius:8,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
      <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {schedule.map((s,i)=>(
          <div key={s.day} style={{display:"grid",gridTemplateColumns:"58px 1fr",gap:10,alignItems:"center",opacity:s.active?1:0.45}}>
            <button onClick={()=>toggle(i)} style={{background:s.active?"#0f172a":"#ffffff",border:`1px solid ${s.active?"#2563eb":"#0f172a"}`,color:s.active?"#1d4ed8":"#64748b",borderRadius:5,padding:"5px 0",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"'Courier New',monospace",textTransform:"uppercase"}}>{s.day}</button>
            {s.active?(
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <input type="time" value={s.start} onChange={e=>setTime(i,"start",e.target.value)} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:5,padding:"5px 8px",color:"#0f172a",fontSize:13,outline:"none",flex:1}}/>
                <span style={{color:"#64748b",fontSize:11}}>→</span>
                <input type="time" value={s.end} onChange={e=>setTime(i,"end",e.target.value)} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:5,padding:"5px 8px",color:"#0f172a",fontSize:13,outline:"none",flex:1}}/>
                <span style={{color:"#64748b",fontSize:11,fontFamily:"'Courier New',monospace",minWidth:36,textAlign:"right"}}>{calcHours([s]).toFixed(1)}h</span>
              </div>
            ):(
              <div style={{fontSize:12,color:"#94a3b8",fontFamily:"'Courier New',monospace",paddingLeft:4}}>— Off</div>
            )}
          </div>
        ))}
      </div>
      <div style={{marginTop:10,fontSize:13,color:"#7c3aed",fontFamily:"'Courier New',monospace",textAlign:"right",fontWeight:700}}>{calcHours(schedule).toFixed(1)} hrs / week</div>
    </div>
  );
}
function ShiftCell({s}) {
  if(!s||!s.active) return <div style={{background:"#f1f5f9",height:"100%",minHeight:52,borderRadius:4}}/>;
  const isNight=parseInt(s.start)>=18||parseInt(s.end)<=8;
  const bg=isNight?"#ede9fe":"#dbeafe";
  const accent=isNight?"#7c3aed":"#2563eb";
  return (
    <div style={{background:bg,border:`1px solid ${accent}33`,borderRadius:4,padding:"6px 8px",minHeight:52,display:"flex",flexDirection:"column",justifyContent:"center",gap:2}}>
      ...
      <div style={{fontSize:10,color:"#64748b",fontFamily:"'Courier New',monospace"}}>{calcHours([s]).toFixed(0)}h</div>
    </div>
  );
}
...
export default function App() {
  ...
  const nav=(id,label)=>(
    <button onClick={()=>setView(id)} style={{background:view===id?"#0f172a":"transparent",border:view===id?"1px solid #1e6fb8":"1px solid transparent",color:view===id?"#1d4ed8":"#64748b",borderRadius:6,padding:"7px 18px",cursor:"pointer",fontSize:13,fontWeight:600,letterSpacing:"0.04em",fontFamily:"'Courier New',monospace",textTransform:"uppercase"}}>{label}</button>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f1f5f9",color:"#0f172a",fontFamily:"'DM Sans','Segoe UI',sans-serif",paddingBottom:60,color:"#0f172a"}}>
      ...
      <div style={{borderBottom:"1px solid #1e3a5f",padding:"0 32px",display:"flex",alignItems:"center",justifyContent:"space-between",height:64}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button onClick={loadPosts} style={{background:"transparent",border:"1px solid #1e3a5f",borderRadius:6,color:"#64748b",padding:"7px 14px",cursor:"pointer",fontSize:12,fontFamily:"'Courier New',monospace"}}>↻ REFRESH</button>
          <button onClick={()=>{setForm(BLANK);setEditTarget(null);setShowAdd(true);}} style={{background:"#0f172a",border:"1px solid #1e6fb8",borderRadius:6,color:"#1d4ed8",padding:"8px 20px",cursor:"pointer",fontSize:13,fontWeight:700,letterSpacing:"0.06em",fontFamily:"'Courier New',monospace",textTransform:"uppercase"}}>+ Add Post</button>
        </div>
      </div>
      ...
      <div style={{padding:"28px 32px 0"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:28}}>
          <StatCard label="Open Posts" value={loading?"…":open.length} sub="Requires staffing" accent="#2563eb"/>
          ...
          <StatCard label="Hrs/Wk Needed" value={loading?"…":Math.round(totalHrs)} sub="Total open post hours" accent="#7c3aed"/>
        </div>
        ...
        {(view==="open"||view==="filled")&&(
          <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
            {[{l:"Type",v:ft,s:setFt,o:POST_TYPES},{l:"Priority",v:fp,s:setFp,o:PRIORITIES}].map(f=>(
              <select key={f.l} value={f.v} onChange={e=>f.s(e.target.value)} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:6,padding:"7px 12px",color:"#64748b",fontSize:12,outline:"none",fontFamily:"'Courier New',monospace"}}>
                <option value="All">{f.l}: All</option>
                {f.o.map(o=><option key={o}>{o}</option>)}
              </select>
            ))}
            <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:6,padding:"7px 12px",color:"#64748b",fontSize:12,outline:"none",fontFamily:"'Courier New',monospace",marginLeft:"auto"}}>
        ...
        {!loading&&(view==="open"||view==="filled")&&(
          <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:10,overflow:"hidden"}}>
            <div style={{display:"grid",gridTemplateColumns:"2fr 1.3fr 1.1fr 70px 100px 90px 130px",padding:"10px 20px",borderBottom:"1px solid #1e3a5f",gap:12}}>
              {["Client / Site","Location","Post Type","Days","Schedule","Priority",""].map(h=>(
                <div key={h} style={{fontSize:10,color:"#64748b",letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"'Courier New',monospace",fontWeight:700}}>{h}</div>
              ))}
            </div>
            {filtered.length===0&&<div style={{padding:"40px",textAlign:"center",color:"#64748b",fontSize:14}}>No posts match this filter.</div>}
            {filtered.map((post,i)=>{
              ...
              return (
                <div key={post.id} style={{display:"grid",gridTemplateColumns:"2fr 1.3fr 1.1fr 70px 100px 90px 130px",padding:"13px 20px",gap:12,alignItems:"center",borderBottom:i<filtered.length-1?"1px solid #0f1f30":"none",background:i%2===0?"transparent":"#f1f5f9",borderLeft:`3px solid ${uc}`}}>
                  <div>
                    <div style={{fontWeight:600,fontSize:14,color:"#0f172a"}}>{post.client}</div>
                    {post.notes&&<div style={{fontSize:11,color:"#64748b",marginTop:2,fontStyle:"italic"}}>{post.notes}</div>}
                  </div>
                  <div style={{fontSize:13,color:"#64748b"}}>{post.location}</div>
                  <div style={{fontSize:13,color:"#64748b"}}>{post.type}</div>
                  <div style={{fontSize:20,fontWeight:800,color:uc,fontFamily:"'Bebas Neue',sans-serif"}}>{days}d</div>
                  <div>
                    <div style={{fontSize:15,fontWeight:700,color:"#7c3aed",fontFamily:"'Bebas Neue',sans-serif"}}>{hrs.toFixed(0)}h/wk</div>
                    <div style={{fontSize:10,color:"#64748b",fontFamily:"'Courier New',monospace",marginTop:1}}>{activeDays.join(" · ")}</div>
                  </div>
                  <div><Badge label={post.priority} color={post.priority==="Critical"?"#ef4444":post.priority==="High"?"#f59e0b":"#22c55e"}/></div>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {view==="open"?(
                      <>
                        <button onClick={()=>setSchedTarget(post)} style={{background:"#ede9fe",border:"1px solid #6d28d9",color:"#7c3aed",borderRadius:5,padding:"4px 7px",cursor:"pointer",fontSize:10,fontWeight:700}}>SCHED</button>
                        <button onClick={()=>startEdit(post)} style={{background:"#dbeafe",border:"1px solid #1e3a5f",color:"#1d4ed8",borderRadius:5,padding:"4px 7px",cursor:"pointer",fontSize:10,fontWeight:700}}>EDIT</button>
                        <button onClick={()=>{setFillTarget(post);setFillForm({filledBy:"",filledDate:new Date().toISOString().slice(0,10)});}} style={{background:"#f0fdf4",border:"1px solid #166534",color:"#22c55e",borderRadius:5,padding:"4px 7px",cursor:"pointer",fontSize:10,fontWeight:700}}>FILL</button>
                        <button onClick={()=>doDelete(post.id)} style={{background:"#fef2f2",border:"1px solid #7f1d1d",color:"#ef4444",borderRadius:5,padding:"4px 7px",cursor:"pointer",fontSize:10}}>✕</button>
                      </>
                    ):(
                      <div>
        ...
        {!loading&&view==="schedule"&&(
          <div>
            <div style={{fontSize:12,color:"#64748b",marginBottom:16,fontFamily:"'Courier New',monospace"}}>
            ...
            <div style={{overflowX:"auto"}}>
              <div style={{minWidth:860}}>
                <div style={{display:"grid",gridTemplateColumns:"210px repeat(7,1fr)",gap:5,marginBottom:5}}>
                  <div/>
                  {DAYS.map(d=>(
                    <div key={d} style={{textAlign:"center",padding:"8px 4px",background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:6}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#1d4ed8",fontFamily:"'Bebas Neue',sans-serif",letterSpacing:"0.08em"}}>{d}</div>
                    </div>
                  ))}
                </div>
                {open.map(post=>{
                  ...
                  return (
                    <div key={post.id} style={{display:"grid",gridTemplateColumns:"210px repeat(7,1fr)",gap:5,marginBottom:5}}>
                      <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderLeft:`3px solid ${uc}`,borderRadius:6,padding:"8px 12px",display:"flex",flexDirection:"column",justifyContent:"center",gap:3,cursor:"pointer"}} onClick={()=>setSchedTarget(post)}>
                        <div style={{fontSize:13,fontWeight:600,color:"#0f172a",lineHeight:1.2}}>{post.client}</div>
                        <div style={{fontSize:11,color:"#64748b"}}>{post.type}</div>
                        <div style={{fontSize:11,color:"#7c3aed",fontFamily:"'Courier New',monospace",marginTop:2}}>{calcHours(sched).toFixed(0)}h/wk</div>
                      </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"210px repeat(7,1fr)",gap:5,marginTop:10}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:10}}>
                    <span style={{fontSize:10,color:"#64748b",fontFamily:"'Courier New',monospace",textTransform:"uppercase",letterSpacing:"0.08em"}}>Day Total</span>
                  </div>
                  {DAYS.map(d=>{
                    const h=open.reduce((sum,p)=>{const s=(p.schedule||[]).find(x=>x.day===d);return sum+(s&&s.active?calcHours([s]):0);},0);
                    return (
                      <div key={d} style={{textAlign:"center",padding:"8px 4px",background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:6}}>
                        <div style={{fontSize:17,fontWeight:800,color:h>0?"#7c3aed":"#0f172a",fontFamily:"'Bebas Neue',sans-serif"}}>{h>0?h.toFixed(0)+"h":"—"}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{display:"flex",gap:20,marginTop:18,alignItems:"center"}}>
              <span style={{fontSize:11,color:"#64748b",fontFamily:"'Courier New',monospace",textTransform:"uppercase",letterSpacing:"0.08em"}}>Legend:</span>
              {[{bg:"#dbeafe",ac:"#2563eb",label:"Day shift"},{bg:"#ede9fe",ac:"#7c3aed",label:"Night shift"},{bg:"#f1f5f9",ac:"",label:"Off"}].map(({bg,ac,label})=>(
                <div key={label} style={{display:"flex",gap:6,alignItems:"center"}}>
                  <div style={{width:14,height:14,background:bg,border:`1px solid ${ac||"#0f172a"}33`,borderRadius:3}}/>
        ...
        {!loading&&view==="dashboard"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
            <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:10,padding:24}}>
              ...
              {byType.length===0?<div style={{color:"#64748b",fontSize:13}}>No open posts.</div>:byType.map(({t,n})=>(
                <div key={t} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:13,color:"#64748b"}}>{t}</span><span style={{fontSize:13,fontWeight:700,color:"#1d4ed8",fontFamily:"'Courier New',monospace"}}>{n}</span></div>
                  <div style={{height:6,background:"#0f172a",borderRadius:3}}><div style={{height:"100%",borderRadius:3,background:"#2563eb",width:`${(n/open.length)*100}%`}}/></div>
                </div>
              ))}
            </div>
            <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:10,padding:24}}>
              ...
              {byLoc.map(({l,n})=>(
                <div key={l} style={{marginBottom:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:13,color:"#64748b"}}>{l}</span><span style={{fontSize:13,fontWeight:700,color:"#f59e0b",fontFamily:"'Courier New',monospace"}}>{n}</span></div>
                  <div style={{height:6,background:"#0f172a",borderRadius:3}}><div style={{height:"100%",borderRadius:3,background:"#f59e0b",width:`${(n/open.length)*100}%`}}/></div>
                </div>
              ))}
            </div>
            <div style={{background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:10,padding:24,gridColumn:"1/-1"}}>
      ...
      <Modal open={showAdd} onClose={()=>{setShowAdd(false);setEditTarget(null);}} wide>
        <div style={{marginBottom:24}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:"0.1em",color:"#0f172a"}}>{editTarget?"Edit Post":"New Open Post"}</div>
          <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{editTarget?"Update details and schedule":"Add a position that needs to be filled"}</div>
        </div>
        ...
        <div style={{display:"flex",gap:12,marginTop:24}}>
          <button onClick={savePost} disabled={saving} style={{flex:1,background:"#0f172a",border:"1px solid #1e6fb8",color:"#1d4ed8",borderRadius:6,padding:"10px",cursor:"pointer",fontWeight:700,fontSize:13,letterSpacing:"0.06em",fontFamily:"'Courier New',monospace",textTransform:"uppercase",opacity:saving?0.6:1}}>{saving?"SAVING…":editTarget?"Save Changes":"Add Post"}</button>
        ...
        {schedTarget&&(
          <>
            <div style={{marginBottom:20}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:"0.1em",color:"#7c3aed"}}>Weekly Schedule</div>
              <div style={{fontSize:13,color:"#64748b",marginTop:4}}>{schedTarget.client} · {schedTarget.type} · {schedTarget.location}</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
              {DAYS.map(day=>{
                const s=(schedTarget.schedule||[]).find(x=>x.day===day)||{day,active:false,start:"06:00",end:"18:00"};
                return (
                  <div key={s.day} style={{display:"grid",gridTemplateColumns:"64px 1fr 60px",gap:12,alignItems:"center",padding:"9px 14px",background:s.active?"#eff6ff":"#f1f5f9",border:`1px solid ${s.active?"#0f172a":"#f8fafc"}`,borderRadius:7,opacity:s.active?1:0.4}}>
                    <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:19,color:s.active?"#1d4ed8":"#64748b"}}>{s.day}</div>
                    {s.active?<div style={{fontFamily:"'Courier New',monospace",fontSize:13,color:"#0f172a"}}>{fmt12(s.start)} – {fmt12(s.end)}</div>:<div style={{fontFamily:"'Courier New',monospace",fontSize:12,color:"#94a3b8"}}>— Off</div>}
                    <div style={{fontFamily:"'Courier New',monospace",fontSize:13,color:"#7c3aed",textAlign:"right"}}>{s.active?calcHours([s]).toFixed(1)+"h":""}</div>
                  </div>
                );
              })}
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:"#f8fafc",border:"1px solid #1e3a5f",borderRadius:8,marginBottom:14}}>
              ...
              <span style={{fontSize:28,fontWeight:800,color:"#7c3aed",fontFamily:"'Bebas Neue',sans-serif"}}>{calcHours(schedTarget.schedule||[]).toFixed(1)} hrs</span>
            </div>
            <div style={{padding:"14px 16px",background:"#ffffff",border:"1px solid #1e3a5f",borderRadius:8,marginBottom:20}}>
              ...
              <div style={{fontSize:14,color:"#0f172a",lineHeight:1.75}}>
                This post requires a <strong>{schedTarget.type}</strong> working <strong>{DAYS.filter(d=>(schedTarget.schedule||[]).find(s=>s.day===d&&s.active)).join(", ")}</strong>. Approximately <strong>{calcHours(schedTarget.schedule||[]).toFixed(0)} hours per week</strong>.{schedTarget.notes&&<> Note: <em>{schedTarget.notes}</em>.</>}
              </div>
            </div>
      ...
      <Modal open={!!fillTarget} onClose={()=>setFillTarget(null)}>
        <div style={{marginBottom:24}}>
          ...
          <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{fillTarget?.client} — {fillTarget?.type}</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
